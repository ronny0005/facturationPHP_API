...
//        $this->modifiePrix($AR_Ref, $AR_Design, $AR_PrixAch, $AR_PrixVen, $prix, $pxMin, $docEntete->DO_Piece, $login);
    ...
    public function ajout_ligneFacturation($qteG, $ARRefG, $cbMarqEntete, $typeFacG, $cattarifG, $prixG, $remiseG, $machinepcG, $acte,$protNo)
    {
        ...
        if($cattarifG == 1) {
            if (($typeFacG != "Vente")
                || ($article->Qte_Gros != 0 && (($article->Qte_Gros <= $qteG) || ($article->Qte_Gros > $qteG && $article->Prix_Min < $prixG)))
                || $article->Qte_Gros == 0) {
            } else {
            ...
            if($typeFacG !="VenteRetour") {
                $artClient = new ArtClientClass($ARRefG, $cattarifG, $this->db);
                if ($docEntete->DO_Domaine == 0 && $cattarifG > 1
                    && (($prixG == $artClient->AC_Coef && $qteG < $artClient->AC_Remise)
                        || ($prixG < $artClient->AC_Coef))) {
                    if (($prixG < $artClient->AC_Coef))
                        $data = array('message' => "Le montant saisie ({$this->objetCollection->formatChiffre($prixG)}) est inférieur au montant minimum ({$this->objetCollection->formatChiffre($artClient->AC_Coef)})");
                    else
                        $data = array('message' => "La quantité saisie ({$this->objetCollection->formatChiffre($qteG)})  est inférieure à la quantité minimum ({$this->objetCollection->formatChiffre($artClient->AC_Remise)})");
                    return json_encode($data);
                }
            }
        }
        if ($typeFacG == "Achat" || $typeFacG == "AchatPreparationCommande"
            || ($typeFacG != "Achat"
                && (($VteNegatif == 1 && ($typeFacG == "BonCommande" || $typeFacG == "PreparationLivraison" || $typeFacG == "BonLivraison"))
                    || (/*$VteNegatif==0 &&*/ ($AS_QteSto + $this->DL_Qte) >= ($qteG)))) || $typeFacG == "Devis"
            || $typeFacG == "PreparationCommande" || $typeFacG == "Avoir" || $typeFacG == "BonCommande" || $typeFacG == "VenteRetour") {
