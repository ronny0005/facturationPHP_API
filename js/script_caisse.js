jQuery(function ($) {
    ...
    function listeReglementCaisse(){
        ...
        $.ajax({
            ...
            dataType: 'html',
            async : false,
            data : "flagAffichageValCaisse="+$("#flagAffichageValCaisse").val()+"&flagCtrlTtCaisse="+$("#flagCtrlTtCaisse").val()+"&Prot_No="+$("#PROT_No").val(),
            success: function (data) {
                $(".reglement").remove();
    ...
    function modifReglement(){
        $("tr[id^='reglement_']").each(function() {
            ...
            var caisseLigne = elem.find("#CA_No").html();
