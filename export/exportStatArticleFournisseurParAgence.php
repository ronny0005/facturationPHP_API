...
$result=$objet->db->requete($objet->depot());     
    ...
    foreach($rows as $row){
        ...
        $result=$objet->db->requete($objet->stat_articleFournisseurParAgence($val, $datedeb, $datefin,$famille,$article,$do_type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $i=0;
        $canetht=0;
        $canetttc=0;
...
<table class="facture">
    <tr style="text-align: center">
        ...
        <th style="width:70px">Référence</th>
            ...
            <th style="">CA <br/>Net HT</th>
            <th style="">Precompte</th>
            <th style="">CA <br/>Net TTC</th>
            <th style="">Qtés<br/> Achetées</th>
        </tr>
    <?php
        if($rows==null){
        }else{
            foreach ($rows as $row){
                ...
                echo "<td>".$row->NumTiers."</td>"
                    ."<td style='width:50px'>".$row->AR_Ref."</td>"
        ...
        echo "<td>Total</td><td></td><td></td><td style='text-align:right'>".$objet->formatChiffre($canetht)."</td>
